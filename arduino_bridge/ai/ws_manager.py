...
class WsManager:
    ...
    async def send_notification(self, message: str, channel: str = "whatsapp"):
        """Send a notification to Tobi via the OpenClaw gateway."""
        if not self._connected:
            logger.warning("Cannot send notification: not connected to Melissa")
            return False
        try:
            await self.send({
                "type": "notify",
    ...
    def notify(self, message: str, channel: str = "whatsapp"):
        """Synchronous wrapper for send_notification."""
        if self._loop and self._connected:
            asyncio.run_coroutine_threadsafe(
                self.send_notification(message, channel),
                self._loop
            )
